function menuOnClick() {
  ...
  // server.js
  // Back-end para sua página: serve arquivos estáticos e responde /buscar e /api/suggestions
  ...
  // Lista de 15 produtos/bandas (ajuste ou adicione imagens conforme sua pasta imgs/)
  const produtos = [
  ...
  // Serve arquivos estáticos (seu index.html, css/, imgs/, etc.)
  app.use(express.static(path.join(__dirname)));
  
  // Rota /buscar (já corresponde ao form action="/buscar" method="get")
  app.get("/buscar", (req, res) => {
    const q = (req.query.q || "").trim().toLowerCase();
  
    // Se a query estiver vazia, redireciona para a home
    if (!q) {
      return res.redirect("/");
    }
    ...
    // Gera HTML simples com resultados (mantém estilo leve)
    let html = `
    <!DOCTYPE html>
    <html lang="pt-BR">
    <head>
    ...
    if (resultados.length === 0) {
      ...
      resultados.forEach(p => {
        // Verifica existência da imagem; se não existir, usa placeholder
  ...
  // Rota de sugestões (JSON) para autocomplete no front-end (opcional)
  ...
  // função simples para escapar HTML (protege pesquisa com caracteres especiais)
